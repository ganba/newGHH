...
mydata<-read.dta("C:/Data/Ethiopia/DataFilesLast/data_for_copula.dta")
attach(mydata)

ww<-cbind(realcons_w1,pcl_w1,AveYrsEducAdults_w1,water_w1,
          electric_w1,toilet_w1,abs(AnyHHMemLackMA_w1-1),
          realcons_w2,pcl_w2,AveYrsEducAdults_w2,water_w2,
          electric_w2,toilet_w2,abs(AnyHHMemLackMA_w2-1))
...
# Cut of for continuous
cut.contin<-c(0.302,0.302,5)
...
# Data type if continuous (0 No, 1 Yes) Benchmark - 10 
DataTypes<-matrix(0,m,1)
for (j in 1:m) {
  if (length(unique(w1[,j]))>8) {
    DataTypes[j,1]<-1
  }
}
...
for (j in 1:m) {
  ...
  if (DataTypes[j,1]==1) {
    ...
    dstat[j,2]<-100*(count(w1[,j])$freq[1]/n)
    dstat[j,3]<-100*(count(w2[,j])$freq[1]/n)
    dstat[j,4]<-dstat[j,3]-dstat[j,2]
    dstat[j,5]<-100*(dstat[j,3]/dstat[j,2]-1)    
  }
}
...
Copula functions provide an intuitive way to test the significance of each component used in the multidimensional measure of well-being. We can drop a component from the copula, and when compare the resulted likelihood with the likelihood of the original copula. In other words, we are comparing a measure with one missing component to a measure which incorporates all components. Table \ref{tbl:LogLikeRatioTest} presents the likelihood ratio test statistics and other relevant indicators: \\
...
for (i in 1:numcol_w1) {
  ...
  KL[i,1]<-param.temp
  KL[i,2]<-sqrt(LL.copula@var.est)
  cop.temp<-archmCopula(family,dim=(numcol_w1-1),param=param.temp)
  dCopi<-dCopula(wave.temp,cop.temp)
  dCopi<-dCopi/sum(dCopi)
  KL[i,3]<-sum(dCopi*log(dCopi/dCop1))
  # Wave 2
  wave.temp<-wave2[,-i]
  LL.copula<-fitCopula(LL.cop, wave.temp, method=opt.meth)
  param.temp<-LL.copula@estimate
  KL[i,5]<-param.temp
  KL[i,6]<-sqrt(LL.copula@var.est)
  cop.temp<-archmCopula(family,dim=(numcol_w1-1),param=param.temp)
  dCopi<-dCopula(wave.temp,cop.temp)
  dCopi<-dCopi/sum(dCopi)
  KL[i,7]<-sum(dCopi*log(dCopi/dCop2))  
}
KL[,4]<-KL[,3]/sum(KL[,3])
KL[,8]<-KL[,7]/sum(KL[,7])

colnames(KL)<-c("Theta1","St.Error","KL1","Weights1","Theta2","St.Error","KL2","Weights2")
rownames(KL)<-DataNames

LLRatio.table<-xtable(KL,digits=2,align="r|cccccccc",
...
As it was expected consumption and asset ownership are the substantial components in our multidimensional measure of well-being. \\

<<Mixed,results='asis'>>=

# cc1.new<-matrix(0,n,1)
# cc2.new<-matrix(0,n,1)
# tm.new<-matrix(0,2,2)
# p.line1<-sum(KL[,3]*temp1)
# p.line2<-sum(KL[,7]*temp2)
# for (i in 1:n) {
#   cc1.new[i,1]<-sum(wave1[i,]*KL[,3])
#   cc2.new[i,1]<-sum(wave2[i,]*KL[,7])
#   if (cc1.new[i,1]>p.line1 & cc2.new[i,1]>p.line2) {
#     tm.new[1,1]<-tm.new[1,1]+1
#   } else if (cc1.new[i,1]>p.line1 & cc2.new[i,1]<p.line2) {
